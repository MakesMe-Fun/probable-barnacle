...
def _build_summary(interest: list[dict], general: list[dict], sending: int, total: int) -> str:
    ...
    if sending < total:
        lines.append(f"_알림에는 상위 {sending}건만 보냅니다. 전체 {total}건은 HTML 리포트에서 확인하세요._")
    return "\n".join(lines)
...
def send_to_discord(
    ...
    to_send = (interest + general)[:max_notifications]

    if not to_send:
        logger.info("Discord로 보낼 이벤트가 없습니다.")
        return

    summary = _build_summary(interest, general, len(to_send), len(eligible))
    if not _post(webhook_url, {"content": summary}):
        return
